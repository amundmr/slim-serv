...
async def update_image(
    ...
    try:
        ...
        char_uuids = [
            c.uuid
            for svc in client.services if svc.uuid.lower().startswith("0000f")
            for c in svc.characteristics
        ]
        if len(char_uuids) < 2:
            raise BleakServiceMissing(f"No vendor service found (got {len(char_uuids)} chars)")

        sorted_uuids = sorted(char_uuids, key=lambda x: int(x[4:8], 16))
        _LOGGER.debug("Discovered %d chars, cmd_uuid=%s  img_uuid=%s",
                      len(sorted_uuids), sorted_uuids[0], sorted_uuids[1])
...
class GiciskyClient:
    ...
    async def read(self, timeout: float = 15.0) -> bytes:
        await wait_for(self.event.wait(), timeout)
        data = self.command_data or b""
        _LOGGER.debug("Received: %s", data.hex())
        return data

    async def write_with_response(self, uuid, packet: bytes, gatt_response: bool = False) -> bytes:
        last_exception = None
        max_retries = 1
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.event.clear()
                await self.write(uuid, packet, gatt_response)
    ...
    async def write_image(self, image: Image, threshold: int, red_threshold: int) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Write failed completely inside state machine loop!")
            return False
        finally:
